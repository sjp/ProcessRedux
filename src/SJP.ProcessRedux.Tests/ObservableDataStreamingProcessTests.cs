...
{
    [TestFixture]
    internal class ObservableDataStreamingProcessTests : ProcessTest
    {
        [Test]
        public static void Ctor_GivenNullConfig_ThrowsArgNullException()
        {
            Assert.Throws<ArgumentNullException>(() => new ObservableDataStreamingProcess(null));
        }
        ...
        public static void State_WhenNotStarted_ThrowsInvalidOperationException()
        {
            ...
            using (var dataProcess = new ObservableDataStreamingProcess(config))
        ...
        {
            var config = new ProcessConfiguration(TestProcessFilePath);
            using (var textProcess = new ObservableDataStreamingProcess(config))
            {
                textProcess.Start();
                Task.Delay(100).Wait();
        ...
        public static void ErrorDataReceived_WhenSubscribed_ReturnsExpectedData()
        {
            ...
            using (var dataProcess = new ObservableDataStreamingProcess(config))
            {
                dataProcess.ErrorData.Subscribe(data => result.AddRange(data));
                dataProcess.Start();
                Task.Delay(1000).Wait();
            }
        ...
        public static void OutputDataReceived_WhenSubscribed_ReturnsExpectedData()
        {
            ...
            using (var dataProcess = new ObservableDataStreamingProcess(config))
            {
                dataProcess.OutputData.Subscribe(data => result.AddRange(data));
                dataProcess.Start();
                Task.Delay(1000).Wait();
            }
